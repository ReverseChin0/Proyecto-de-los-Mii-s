...
Shader "Shader Forge/ShaderMan" {
    Properties {
        ...
        _ColorShoes ("ColorShoes", Color) = (0.5,1,0.5,1)
        _ColorShirt ("ColorShirt", Color) = (0.5,1,1,1)
        _ColorTie ("ColorTie", Color) = (1,0.5,1,1)
        _ColorSuit ("ColorSuit", Color) = (0.5,0.5,1,1)
        _ColorNeck ("ColorNeck", Color) = (1,1,0.5,1)
    }
    SubShader {
        Tags {
            "RenderType"="Opaque"
        ...
        Pass {
            ...
            uniform float4 _ColorShoes;
            uniform float4 _ColorShirt;
            uniform float4 _ColorTie;
            uniform float4 _ColorSuit;
            uniform float4 _ColorNeck;
            struct VertexInput {
...
////// Specular:
                ...
                float node_7577 = node_6886;
                float node_5237 = node_3419;
                float3 node_8889 = (((_ColorSkin.rgb*node_4408)+(_ColorShoes.rgb*node_3933)+(_ColorSuit.rgb*node_1298))+((_ColorShirt.rgb*node_6192)+(_ColorTie.rgb*node_7577)+(_ColorNeck.rgb*node_5237)));
                float3 diffuseColor = node_8889; // Need this for specular when using metallic
